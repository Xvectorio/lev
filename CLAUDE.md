...
- `README.md` is the short introduction and quick start; `docs/guide.md` is the operator-facing spec (stages, agent API bodies, reliability limits). Keep it in sync when behavior changes. The in-app `?` help (`ui/src/Help.svelte`, `TOPICS`) is a condensed copy of the guide: update both.
...
Data flow: **Vector → Caddy (`/loki/api/v1/push`, basic auth) → Loki (`LOG_RETENTION_HOURS`, default 48h) → worker collector → PostgreSQL incidents → Jev triage → agent proposal → operator approval → agent verification → recurrence observation → resolved.**
