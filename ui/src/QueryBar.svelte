...
<div class="search-field" role="group" aria-label="Query" onfocusout={(e) => { if (!(e.currentTarget as HTMLElement).contains(e.relatedTarget as Node)) suggesting = false; }}>
  ...
  <div class="query-box">
    ...
    {#if Object.keys(filters).length || text}<button type="button" class="clear icon-button" aria-label="Clear query" title="Clear query" onclick={() => { filters = {}; text = ''; input?.focus(); }}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3 4h14l-5.5 7v6l-3 2v-8z"/><path d="M16 14l5 5M21 14l-5 5"/></svg></button>{/if}
